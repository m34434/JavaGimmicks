...
public class CompositeListTest
{
   ...
   {
      final List<String> reference = buildReference(_l1, _l2, _l3, _l4, _l5);

      assertEquals(reference.size(), _composite.size());

      final Comparator<List<String>> comparator = ListComparator.getComparableInstance();
   ...
   {
      // This will especially test, if the internal ListIterator works right in
      // the reverse way

      final List<String> referenceReverse = ReverseList.decorate(buildReference(_l1, _l2, _l3, _l4, _l5));
      final List<String> compositeReverse = ReverseList.decorate(_composite);
   ...
   {
      final ListIterator<String> it = _composite.listIterator(3);
   ...
   private static <E> List<E> buildReference(final List<? extends E>... lists)
   {
      ...
      for (final List<? extends E> c : lists)
      {
         result.addAll(c);
      }
